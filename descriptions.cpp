...
uint32_t parse_descriptions(dungeon_t *d)
{
  ...
  retval = 0;
  file = "saved_dungeons//monster_desc.txt";
  //file = getenv("HOME");
  if (file.length() == 0) {
    file = ".";
  }
  ...
  f.close();
  file = "saved_dungeons//object_desc.txt";
  // file = getenv("HOME");
  if (file.length() == 0) {
    file = ".";
  }
...
uint32_t print_descriptions(dungeon_t *d)
{
 ...
 // pair_t p;
  ...
  for (int i=0;i<15;i++){
    ...
    if((b/80>=1 && uniqC !=1)||(b/80<1)){
      printf("%c =  %s    %d \n",m[i].symbol,m[i].name.c_str(),b);
      if(b/80>=1){uniqC++;}
    }


    // printf("%c =  %s  \n",object_symbol[o[i].type],o[i].name.c_str(),o[i].);
     // o2->symbol =object_symbol[o[i].type];
     // d->object_map[i][i] =o2;
     // oi++;
...
void gen_monsters2(dungeon *d){
  ...
  for (i2 = sum = 0; i2 < d->num_rooms; i2++) {
    if (!pc_in_room(d, i2)) {
      sum += d->rooms[i2].size[dim_y] * d->rooms[i2].size[dim_x];
    }
  }
 ...
 d->num_monsters = std::min(d->max_monsters,sum);
 printf("sum  %d  max:%d   \n",sum,d->max_monsters);

 for (i2 = 0; i2 < d->num_monsters; i2++) {                      //------------------ gen loop
   m2 = new npc;
   descRan = (rand()%(int)m.size());
  genRan =(rand()%100)+1;
  monstRARE = m[descRan].rarity;
  int b = m[descRan].abilities;
  ...
  /*
    ...
    else if(b/80<1){printf("%c =  %s    %d \n",m[i].symbol,m[i].name.c_str(),b);}
  */


  if(genRan<= (int)monstRARE){
    printf("%d.)     %c =  %s   color = %d   \n ",i2+1,m[descRan].symbol,m[descRan].name.c_str(),m[descRan].color[0]);
  printf("              gen_Ran: %d  desc_RARE: %d   \n",genRan,monstRARE);


  // create the monst here.
  ...
  do {                                                            // placement info
    ...
    m2->position[dim_y] = p[dim_y];
    m2->position[dim_x] = p[dim_x];
    
    d->character_map[p[dim_y]][p[dim_x]] = m2;
    m2->speed = m[descRan].speed.roll();                                
    m2->alive = 1;
    m2->sequence_number = ++d->character_sequence_number;         
    m2->characteristics = rand() & 0x0000000f;                    // we have these
   // m->npc->characteristics = 0xf;//
    m2 ->damage = m[descRan].damage;
    m2 ->hitpoints = m[descRan].hitpoints.roll();
    
    m2->color =  m[descRan].color[0];
    m2->symbol = m[descRan].symbol;
    m2->have_seen_pc = 0;
    m2->kills[kill_direct] = m2->kills[kill_avenged] = 0;

    d->character_map[p[dim_y]][p[dim_x]] = m2;

    heap_insert(&d->events, new_event(d, event_character_turn, m2, 0));
...
void gen_objects2(dungeon *d){
 ...
 for (int i = 0; i < 10; i++) {                      //------------------ gen loop
   o2 = new object;
   //memset(m2, 0, sizeof (*m2));

   descRan = (rand()%(int)o.size());
   //descRan = 19;
  genRan =(rand()%100)+1;
  objRARE = o[descRan].rarity;
  
  if(o[descRan].artifact && artCount>=1){genRan=200;}

  if(genRan<= (int)objRARE ){ 
    o2->symbol =object_symbol[o[descRan].type];
    

    printf("%d.)     %c =  %s   color = %d   \n ",i+1,o2->symbol,o[descRan].name.c_str(),o[descRan].color);
    printf("              gen_Ran: %d  desc_RARE: %d   \n",genRan,objRARE);
    i++;
  ...
  this->attribute = attrubute;
    */	
    ...
    o2->color =  o[descRan].color;
    o2->speed = o[descRan].speed.roll();
    o2->hitpoints = o[descRan].hit.roll();
    o2-> damage = o[descRan].damage;
    o2->dodge = o[descRan].dodge.roll();
    o2->defence = o[descRan].defence.roll();
    o2->weight = o[descRan].weight.roll();
    o2->value = o[descRan].value.roll();
    o2-> art = o[descRan].artifact;
    o2-> attrubute = o[descRan].attribute.roll();

    if(o[descRan].artifact){artCount++;}
...
void gens(dungeon *d)
{
  ...
  for (i = 0; i < d->num_monsters; i++) {                           // char gen loop
    m = new npc;
    memset(m, 0, sizeof (*m));
